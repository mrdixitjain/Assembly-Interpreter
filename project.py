...
commands={'101': 'print', '102': 'add', '103': 'sub', '104': 'mul', '105': 'div', '106': 'mod', '107': 'inc', '108': 'dec',
		 '109': 'band', '110': 'bor',
...
while(cc<len(inputline)):
	...
	#print operation=> print A/B/C/D/E/F
		...
		if(len(line)!=2):
			exit0('invalid number of arguments at line number '+str(cc))
	...
	# hlt operation=> terminate the execution of program
		...
		if(len(line)!=1):
			...
			exit0('execution completed at line number '+str(cc))
	...
	# sta operation, move value stored into given register at given memory address  => sta A 1000 => [1000]=A
		...
		if(len(line)!=3):
			...
			exit0('invalid argument at line number '+str(cc)+'. Please enter a register.')
		elif(int(line[2])>=2000 or int(line[2])<0):
			exit0('memory address out of range. Error at line number '+str(cc))
		else:
	...
	elif(line[0] in ['205', '206', '207', '208', '209']):
		exit0('this instruction can only be used with compare statement. Error at line number '+str(cc))
